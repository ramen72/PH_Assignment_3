...
create table matches(
  ...
  base_ticket_price smallint not null,
...
(106, 'Real Madrid vs Barcelona', 'Champions League', 110.00, 'Postponed'),
...
update bookings set payment_status = 'Action Required' where payment_status is null;



-- Query 4: Retrieve match booking details along with the User's full name and the scheduled Match fixture teams.
select booking_id, full_name, fixture, total_cost from bookings inner join users on bookings.user_id = users.user_id inner join matches on bookings.match_id = matches.match_id
...
select * from matches order by base_ticket_price desc offset 1 limit 2
